...
\subsection{Historical Analysis Library}

To transition the framework from a simple execution tool to a comprehensive analytics platform, a \textbf{Historical Analysis Library} was implemented in the frontend. This module allows users to access and visualize results from previous executions without re-triggering the computationally expensive Hadoop or NoSQL pipelines.

\begin{itemize}
  \item \textbf{Persistence Layer:} The library queries the PostgreSQL \texttt{run\_metadata} table to identify all successful historical runs.
  \item \textbf{Smart Filtering:} It utilizes a \texttt{DISTINCT ON (dataset\_name, pipeline\_name)} query logic, ensuring that users are presented with the \textbf{latest} result for every unique combination of dataset and processing engine.
  \item \textbf{On-Demand Analysis:} When a historical run is selected, the frontend triggers the \texttt{MasterAnalyzer} via the background listener. The system regenerates the performance charts (Data Quality, Pipeline Comparison, and Batch Timings) using the existing database records and updates the UI dashboard in real-time.
\end{itemize}
...
The database employs a fully normalized, Third Normal Form (3NF) relational hierarchy, dividing metadata into three distinct tiers: Pipeline Runs (\texttt{run\_metadata}), Batch Processing (\texttt{batch\_metadata}), and Query Performance (\texttt{query\_metadata}). The schema utilizes a Composite Primary Key (\texttt{run\_id, batch\_no}) for batches to prevent artificial surrogate keys, thereby eliminating costly \texttt{JOIN} operations when analyzing batch-level query trends. Three separate result tables---one per mandatory query---hold the aggregated data outputs, referencing the overall \texttt{run\_id}.
...
\midrule
run\_id                 & SERIAL PK    & Auto-incremented unique identifier for the entire pipeline run \\
pipeline\_name          & VARCHAR(50)  & Execution engine (e.g., \texttt{mapreduce}, \texttt{mongodb}) \\
dataset\_name           & VARCHAR(255) & The name or path of the selected dataset \\
avg\_batch\_size        & DOUBLE       & Calculated average batch size across the dataset \\
total\_runtime          & DOUBLE       & Total wall-clock execution time for all batches \\
total\_malformed\_record\_count & BIGINT & Aggregate sum of logs that failed regex parsing \\
total\_record\_count    & BIGINT       & Aggregate sum of all logs processed \\
execution\_timestamp    & TIMESTAMP    & UTC datetime when the run was recorded \\
\bottomrule
\end{tabularx}
\end{schematable}

\subsection{Batch Table: \texttt{batch\_metadata}}

Stores performance and execution data strictly for individual batch chunks processed during a pipeline run.
...
\textnormal{\textbf{Column}} & \textbf{Type} & \textbf{Description} \\
\midrule
run\_id                 & INTEGER FK & References \texttt{run\_metadata.run\_id} (Part of Composite PK) \\
batch\_no               & INTEGER    & Sequential batch identifier (e.g., 1, 2, 3) (Part of Composite PK) \\
record\_count           & BIGINT     & Total records processed in this specific batch \\
malformed\_record\_count & BIGINT     & Number of malformed records found in this batch \\
batch\_runtime          & DOUBLE     & Wall-clock execution time for this specific batch \\
\bottomrule
\end{tabularx}
\end{schematable}
...
hosts_list           & TEXT       & Comma-separated list of host IP addresses \\
\bottomrule
\end{tabularx}
\end{schematable}

\subsection{Query Timing Table: \texttt{query\_metadata}}

Stores the individual execution times for each analytical query within a specific batch, enabling granular performance analysis. By referencing the composite key \texttt{(run\_id, batch\_no)}, it completely negates the need for \texttt{JOIN} statements when performing batch-wise trend analysis.
...
\textnormal{\textbf{Column}} & \textbf{Type} & \textbf{Description} \\
\midrule
query\_meta\_id      & SERIAL PK  & Unique identifier for the performance entry \\
run\_id              & INTEGER FK & Composite FK reference to \texttt{batch\_metadata} \\
batch\_no            & INTEGER FK & Composite FK reference to \texttt{batch\_metadata} \\
query\_number        & INTEGER    & The query being timed (1, 2, or 3) \\
query\_runtime       & DOUBLE     & Precise execution time in milliseconds \\
\bottomrule
\end{tabularx}
\end{schematable}

% ══════════════════════════════════════════════════════════════════════════════
%  SECTION 7 — ADVANCED ANALYTICS MODULE
% ══════════════════════════════════════════════════════════════════════════════
\section{Advanced Analytics Module}

To deeply understand the performance characteristics, data integrity, and scalability of the big data pipelines, a dedicated set of Java-based analyzers was implemented. These analyzers run automatically upon pipeline completion, fetching metrics from the relational database and generating visual graphs using \texttt{JFreeChart}.

\subsection{Data Quality Analyzer}
The \texttt{DataQualityAnalyzer} monitors the integrity of the parsing phase. It aggregates the \texttt{record\_count} and \texttt{malformed\_record\_count} from the \texttt{batch\_metadata} table to calculate the overall data parsing success rate. A pie chart is generated to visually contrast successfully parsed log entries against malformed entries, allowing administrators to instantly identify format drift or corruption in raw logs.

\vspace{5cm} % Placeholder for graph

\subsection{Pipeline Comparison Analyzer}
The \texttt{PipelineComparisonAnalyzer} focuses on macroscopic performance. It queries the \texttt{run\_metadata} table to compare the \texttt{total\_runtime} of the current pipeline execution against previous executions using different engines (e.g., MapReduce vs. MongoDB vs. Hive) on the same base dataset. This enables empirical conclusions about which processing engine performs best for overall batch execution.

\vspace{5cm} % Placeholder for graph

\subsection{Query Timing Analyzer}
The \texttt{QueryTimingAnalyzer} provides microscopic performance insights into both sequential batch execution and internal query latency. It generates three distinct visualizations:
\begin{itemize}
    \item \textbf{Internal Query Timings:} A chart detailing the execution time of Query 1, Query 2, and Query 3 for the current run, highlighting which analytic operation is the most computationally expensive.
    \item \textbf{Sequential Batch Trends:} A line graph plotting runtime across sequential dataset chunks (batches), which is critical for identifying memory leaks, disk I/O bottlenecks, or JVM garbage collection pauses that worsen over time.
    \item \textbf{Cross-Pipeline Query Timings:} A comparative chart that evaluates the runtime of specific queries across different processing engines, identifying if an engine excels at aggregation but struggles with sorting.
\end{itemize}

\vspace{5cm} % Placeholder for graph

\subsection{Batch Size Impact Analyzer}
The \texttt{BatchSizeImpactAnalyzer} is a highly advanced scalability evaluator. It groups historical pipeline executions by the target batch size configured by the user (e.g., chunks of 500 lines vs. 10,000 lines). By analyzing this tradeoff, it generates three critical visualizations:
\begin{itemize}
    \item \textbf{Total Runtime Impact (Line Chart):} Demonstrates the overhead penalty of initializing the processing engine too many times on very small batches.
    \item \textbf{Throughput Scalability (Bar Chart):} Calculates records processed per second ($Total Records / Total Runtime$) to find the optimal batch size configuration for peak system efficiency.
    \item \textbf{Average Batch Latency (Line Chart):} Shows the linear or non-linear scaling of time required to process an individual chunk as its size increases.
\end{itemize}

\vspace{5cm} % Placeholder for graph

% ══════════════════════════════════════════════════════════════════════════════
%  SECTION 8 — EQUIVALENCE ACROSS PIPELINES
% ══════════════════════════════════════════════════════════════════════════════
