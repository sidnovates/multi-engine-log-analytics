...
public class Q3Hive_V2 {

    private static final String HQL_SCRIPT  = "Query3/Hive/q3_hourly_error_v2.hql";
    private static final String HDFS_BASE   = "/user/nasa_etl";

    public static void main(String[] args) throws Exception {
        if (args.length < 2) {
            System.out.println("Usage: java Query3.Hive.Q3Hive_V2 <input_dir> <run_id>");
            System.exit(1);
        }
    ...
    public static void run(String hdfsInputDir, int runId) throws Exception {
        ...
        String hdfsOutputDir = HDFS_BASE + "/hive_output/q3_batch_" + runId;

        Path outPath = new Path(hdfsOutputDir);
        if (fs.exists(outPath)) fs.delete(outPath, true);

        System.out.println("Executing Query 3 (Hive V2)...");
        ...
        System.out.println("\nQuery 3 Runtime (Hive V2 + SQL): " + (endTime - startTime) + " ms");
    }
